...
function assertContains(source, values) {
  values.forEach(value => assert.ok(source.includes(value), `missing runtime contract: ${value}`));
}
...
test('CRM and WhatsApp visual redesign preserve operational queue contracts', () => {
  ...
  assertContains(whatsapp, ['page-whatsapp','wpp-sync','wpp-kpis','wpp-filtro-status','wpp-filtro-tipo','wpp-tabela']);
...
test('settings redesign preserves identity users security backup and updater ids', () => {
  ...
  assertContains(settings, ['desktop-updater-settings','updater-status-text','updater-current-version','updater-check-btn','updater-download-btn','updater-install-btn']);
...
test('import and audit redesign preserve admin-only workflow contracts', () => {
  assertContains(imports, ['page-importar','import-select-file','import-cancel','import-file-name','import-preview-card','import-summary','import-preview-body','import-confirm','import-cancel-bottom']);
  assertContains(audit, ['page-auditoria','audit-refresh','audit-entity','audit-action','audit-table-body']);
