...
test('runs migrations once, records schema version and is idempotent', async () => {
  ...
  assert.equal(audits.length, expectedVersions.length);
  const migrationRows = db.exec('SELECT version,name FROM schema_migrations ORDER BY version');
  assert.deepEqual(migrationRows[0].values.map(row => row[0]), expectedVersions);
  ...
  assert.equal(db.exec('SELECT COUNT(*) FROM schema_migrations')[0].values[0][0], expectedVersions.length);
  db.close();
});
