(function (root) {
  ...
  function clinicAdapter() {
    return { query: clinicStore.query, run: clinicStore.run };
  }

  function professionalAdapter(database = professionalDb) {
    return {
      query: (sql, params) => querySqlJs(database, sql, params),
  ...
  function queueProfessionalSave() {
    const api = desktopIsolationApi();
    if (!api?.saveProfessional || !professionalDb || !accessSession?.token) {
      return Promise.resolve({ ok: true, skipped: true });
    }
    const bytes = Array.from(professionalDb.export());
    const token = accessSession.token;
    professionalSaveQueue = professionalSaveQueue
      .catch(() => {})
  ...
  function shouldRouteClinical(sql) {
    return !bootstrapMode && Boolean(accessSession) && isolation.touchesClinicalTable(sql);
  }

  function assertProfessionalClinicalSession() {
    if (!isolation.canUseClinicalDatabase(accessSession?.role) || !professionalDb) {
  ...
  function copySharedTableToProfessional(table) {
    ...
    if (rows.length) {
      ...
      const stmt = professionalDb.prepare(`INSERT INTO ${table} (${columns.join(',')}) VALUES (${placeholders})`);
      try {
        rows.forEach(row => {
  ...
  async function initializeCanonicalClinic() {
    ...
    if (Array.isArray(loaded?.data) && loaded.data.length) {
      ...
      await api.saveClinic(Array.from(clinicStore.export() || []));
    }
  }
  ...
  function run(sql, params = []) {
    if (shouldRouteClinical(sql)) {
      ...
      // Entrega A mantém um shadow clínico central criptografado somente para compatibilidade
      // com o backup/restore atual. A UI e as leituras clínicas nunca usam esse shadow.
  ...
  async function createProfessionalDatabase(session) {
    ...
    clinicStore.run(`INSERT OR REPLACE INTO data_isolation_migrations
      ...
      JSON.stringify({ source: 'clinic-shadow', patientIds: summary.patientIds })
    ]);
    await queueClinicSave();
    return summary;
  }
